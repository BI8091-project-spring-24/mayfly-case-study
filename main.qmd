...
Load cleaned datafiles. Alternative to running all the cleaning steps.
...
To limit datapoints with spatial inaccuracies, we want to see how far the _Baetis rhodani_ datapoints typically are from freshwater. Since the speices is found in running water in its larval stages, the Norwagian river network (Elvenett) will be used to calculate the distance. Then the distribution of distances to rivers will be plotted. This information will aid us in the next step, which is to limit the raster-layer used in the SDM. 
